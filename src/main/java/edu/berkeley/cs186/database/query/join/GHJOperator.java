...
public class GHJOperator extends JoinOperator {
    ...
    private void buildAndProbe(Partition leftPartition, Partition rightPartition) {
        ...
        for (Record build : buildRecords) {
            DataBox value = build.getValue(buildColumnIndex);
            if (!hashTable.containsKey(value)) {
                hashTable.put(value, new ArrayList<>());
            }
            hashTable.get(value).add(build);
        }

        for (Record probe : probeRecords) {
            DataBox value = probe.getValue(probeColumnIndex);
            ...
            for (Record buildRecord: hashTable.get(value)) {
                Record join;
                if (probeFirst) {
                    join = probe.concat(buildRecord);
                } else {
                    join = buildRecord.concat(probe);
                }

                this.joinedRecords.add(join);
            }
        }
